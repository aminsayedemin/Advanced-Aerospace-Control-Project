...
% omb = 1; % [rad/s] % Lower bound on crossover frequency of S
...
s = zpk('s');
F2 = gain * om^2/(s^2 + 2*csi*om*s + om^2);
S_des = 1 - F2;
% S_des = F2;
F2 = c2d(F2, Ts, 'foh');
...
% Loop = connect(G, Rp, Rphi, W1, W2, W3, {'e_phi'}, {'z_1', 'z_2', 'z_3', 'phi', 'p'}, OPT); % Sum,

P = G(1) * (Rp(1) + Rp(2)*G(2)*Rp(1)/(1-G(2)*Rp(2)));

Loop = augw(P*Rphi, W1, W2, W3);

opt = hinfstructOptions('Display', 'final', 'RandomStart', 10);
K = hinfstruct(Loop, opt);
