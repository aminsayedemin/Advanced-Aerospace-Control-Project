...
K.Blocks.b.Value =    0.005949772860362;
K.Blocks.c1.Value =    1.904864261865997;
K.Blocks.c2.Value =    -0.047796774725830;
K.Blocks.d1.Value =    0.005794190586253;
K.Blocks.d2.Value =    -0.136639331103186;
K.Blocks.d3.Value =    6.084295197359233;
...
for n = 1:N
    Y_v(n) = -0.264 + 3 * 4.837/100*(-0.264)*randn(1,1);
    L_v(n) = -7.349 + 3 * 4.927/100*(-7.349)*randn(1,1);
    Y_d(n) = 9.568 + 3 * 4.647/100*(9.568)*randn(1,1);
    L_d(n) = 1079.339 + 3 * 2.762/100*(1079.339)*randn(1,1);
...
grid on
legend('Uncertain plant', 'Function W', 'Interpreter', 'Latex');

% Robust stability verification
figure;
hold on
p_F = bodeplot(F);
p_Winv = bodeplot(1/INFO.W1);
grid on
legend('Complementary sensitivity', '1/W3', 'Interpreter', 'Latex');
